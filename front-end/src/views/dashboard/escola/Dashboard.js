...
class Dashboard extends Component {
  ...
  componentDidMount() {
    ...
    setInterval(() => { this.loadDashboard(this.state.selectedClasses)}, 120000);
  }

  loadClasses = inicial => {
    api.get_classes().then(result => {
      ...
      result.push({label: 'Escola', value: 'school'});
      
      this.setState({classes: result});
      this.setState({selectedClasses: [result[result.length-1]]});

      if (inicial) {
        this.loadDashboard([result[result.length-1]]);
      }
    });
  }
  ...
  handleChange = (selectedClasses) => {
    if (selectedClasses === null) selectedClasses = [];
      ...
      else if ((selectedClasses.length > 1) && (selectedClasses[selectedClasses.length-2].value === 'school')) {
      ...
      <div>
        ...
        <CCard>
          <CCardBody>
            <CRow>
              <CCol sm="5">
                <h4 id="traffic" className="card-title mb-0">Historico</h4>
